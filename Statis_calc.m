%*****      Observation Well Statistical Parameter Calculation      *****%

function [ Avg,Cv,R, theta]=Statis_calc(Signal,N,winlength);
...
signum=length(Signal);                                          % number of data in the signal
  ...
  for ii=1:win
       Avg(ii,1)=mean(Signal(z:z+winnum-1,1));                                                 % The first feature, average value
       Cv(ii,1)=((var(Signal(z:z+winnum-1,1)))^0.5)/Avg(ii,1);                          % The second feature, the coefficient of variation
       Max(ii,1)=max(Signal(z:z+winnum-1,1));
       Min(ii,1)=min(Signal(z:z+winnum-1,1));
       R(ii,1)=Min(ii,1)/Max(ii,1);                                                                           % The min/max ratio, R
       m(ii,:)=polyfit(regx,Signal(z:z+winnum-1,1),1);
    
        if m(ii,1)<=0
           theta(ii,1)=-atan(m(ii,1))/pi;
       else
           theta(ii,1)=1-(atan(m(ii,1))/pi);
        end
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% z=z+2;
       z=z+1;
  end
